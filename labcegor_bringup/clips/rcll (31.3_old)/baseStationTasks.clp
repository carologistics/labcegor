...
;Problem: nachdem fact modifiziert wurde, wird maschine nochmal instructed, obwohl base schon geholt wurde
(defrule instructBS-2
  ...
  ;?machine-f <- (machine (name M-BS) (state IDLE) (waitingFor FALSE))
...
(defrule machineDoneBS
  ...
  ;(test (or (eq ?machineState IDLE) (eq ?machineState READY-AT-OUTPUT)))
...
;    (printout blue "robotDone" ?idR crlf)
;    else 
;  )
;)

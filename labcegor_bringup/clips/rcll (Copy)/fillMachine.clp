...
=>
  ;(if (or (> ?rs1 0) (> ?rs2 0)) then 
  (printout blue "asserting" ?rs1 ?rs2 crlf)
  (assert (robot2ToBS))
  (assert (robot2working))
  ;else
  ;  (printout blue "noBasesNeeded?" ?rs1 ?rs2 crlf)
  ;) ;only if bases are needed
...
=>
  ...
  (if(and (= 2 ?robotID) (and ?res (= ?tidRobot ?taskID)))
    ...
    ;(printout red "robot2NotDoneFill" crlf)
  )
...
(defrule instructBS2Fill
  ...
  ;?order-tasks-f <- (order-tasks (nextTaskToBeDone 2) (orderID ?oid));nur damit roboter nicht direkt los fährt
...
=>
  ;(if (< ?rs1 ?rs2) then (bind ?ringStation M-RS2) else (bind ?ringStation M-RS1)) ;so ring station zuweisen?

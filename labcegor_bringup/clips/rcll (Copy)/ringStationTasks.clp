...
(defrule placeGedönse-5,10,15
  ...
  ;?machine-f <- (machine (name ?machine) (waitingFor FALSE))
  (protobuf-peer (name ROBOT1) (peer-id ?peer-id)) 
...
=>
  ...
  ;(modify ?machine-f (waitingFor TRUE))
  (retract ?place-f)
...
(defrule instructRS-6,11,16
  ...
  ;?machine-f <- (machine (name ?machine) (state IDLE))
  ...
  ;(not (Robot2CurrentlyAtSlide)) ; wait until robot2 is finished with placing base in slide
  ;(not (Robot3CurrentlyAtSlide)) ; wait until robot2 is finished with placing base in slide
  ;(not (robot3BaseInSlide ?ringStation ?machine))
  
  ;(if (eq ?machine M-RS1) then (bind ?payed ?rs1Payed) else (bind ?payed ?rs2Payed)) ;how much was payed in current machine?
  ;(not (< ?payed ?cost));only if enough was payed
...
=>
  ...
  ;(printout green "after instruction payed: M-RS1: " ?rs1Payed " M-RS2: " ?rs2Payed crlf)  
...
;Next TODO: prüfen was nun passiert? Legt roboter gedönse ab und wird machine instructed?


;einbauen, dass maximal drei teile in station liegen/gezahlt wurden

                ...
                (if (eq ?robo_order 0);; aka was retrive
                    then
                        ...
                        (if (eq ?id 1)
                            ...
                            (printout red "NEXTSTEP UPDATE ORDER" crlf)
                        )
                )      
        )
...
(machine_status (name M-RS1) (slide_shelf ?rs_slide))
=>
(retract ?rt)
(switch ?it
            (case 1 then
            ...
            (case 2 then
                (assert (action (id 2) (a_type "r") (machine M-CS1) (io SHELF) (task_id (+ ?last_robo_task 1)))) ;action unify
                (modify ?robo_s (task (+ ?last_robo_task 1)))
                (modify ?CS_it (iteration (+ ?it 1)))
            )
            (case 3 then
            ...
            (case 5 then
                (if (and (eq ?m_task 0) (eq ?m_pos OUTPUT))
                    then
                        (assert (action (id 2) (a_type "r") (machine M-CS1) (io OUTPUT) (task_id (+ ?last_robo_task 1))));needs finish of machine - if machine status task 0 pos out for the machine the robo is sanding
                        (modify ?robo_s (task (+ ?last_robo_task 1)))
                        (modify ?CS_it (iteration (+ ?it 1)))
                    else
                        ;(modify ?CS_it (iteration 5)) ; loop untill if true
                        (modify ?lc (c_time ?ros-time-float))
...
(defrule request_task_CS2_refill
?rt <- (request_task (id 2) (last_task ?last_robo_task) (robo_order ?last_robo_order) (machine_order ?last_machine_order)) ;(machine_order ?last_machine_order)
?lc <- (last_checked (id 2) (c_time ?check_time))
(time ?ros-time-float)
(test (> (- ?ros-time-float ?check_time) 1))
...
(switch ?it
            ...
            (case 2 then
                (assert (action (id 2) (a_type "r") (machine M-CS2) (io SHELF) (task_id (+ ?last_robo_task 1)))) ;action unify
                (modify ?robo_s (task (+ ?last_robo_task 1)))
                (modify ?CS_it (iteration (+ ?it 1)))
            )
            (case 3 then
            ...
            (case 4 then
                (assert (instruct (machine M-CS2) (operation RETRIEVE_CAP) (task_id  1)));needs finish of robo - easy do together with next m
                (modify ?machine_s (task 1))
            ...
            (case 5 then
                (if (and (eq ?m_task 0) (eq ?m_pos OUTPUT))
                    then
                        (assert (action (id 2) (a_type "r") (machine M-CS2) (io OUTPUT) (task_id (+ ?last_robo_task 1))));needs finish of machine - if machine status task 0 pos out for the machine the robo is sanding
                        (modify ?robo_s (task (+ ?last_robo_task 1)))
                        (modify ?CS_it (iteration (+ ?it 1)))
                    else
                        (modify ?lc (c_time ?ros-time-float))
...
?machine_s <- (machine_status (name ?m_name) (task ?m_task) (order ?m_order) (pos ?m_pos))
?pay_it <- (pay_it (id 3) (iteration ?it))
...
(switch ?it
            ...
            (case 2 then
                (if (and (eq ?m_task 0) (eq ?m_pos OUTPUT))
                    then
                        (assert (action (id 3) (a_type "r") (machine M-BS) (io OUTPUT) (task_id (+ ?last_robo_task 1))));needs finish of machine - if machine status task 0 pos out for the machine the robo is sanding
                        (modify ?robo_s (task (+ ?last_robo_task 1))(order 0))
                        (modify ?pay_it (iteration (+ ?it 1)))            
                    else
                    (modify ?lc (c_time ?ros-time-float))

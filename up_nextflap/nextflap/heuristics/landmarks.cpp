...
bool LandmarkRPG::verifyFluent(TVariable v, TValue value, TState *s,
                               SASTask *task) {
  this->task = task;
  initialize(s);
...
bool LandmarkRPG::verifyFluents(std::vector<TVariable> *v,
                                std::vector<TValue> *value, TState *s,
                                SASTask *task) {
  this->task = task;
  initialize(s);
...
bool LandmarkRPG::verifyActions(std::vector<SASAction *> *actions, TState *s,
                                SASTask *task) {
  this->task = task;
  initialize(s);
  while (remainingGoals.size() > 0 && lastLevel->size() > 0) {
    ...
    for (unsigned int i = 0; i < lastLevel->size(); i++) {
      ...
      for (unsigned int j = 0; j < aList.size(); j++) {
        SASAction *a = aList[j];
        if (!achievedAction[a->index] && isExecutable(a) &&
            allowedAction(a, actions)) {
          achievedAction[a->index] = true;
          addActionEffects(a);
        }
      }
...
bool LandmarkRPG::isExecutable(SASAction *a, TVariable v, TValue value) {
  ...
  for (SASConditionalEffect &e : a->conditionalEff) {
    ...
    for (unsigned int j = 0; j < e.startCond.size(); j++)
      if (!fluentAchieved(e.startCond[j].var, e.startCond[j].value)) {
        executable = false;
        break;
      }
    for (unsigned int j = 0; j < e.endCond.size(); j++)
      if (!fluentAchieved(e.endCond[j].var, e.endCond[j].value)) {
        executable = false;
        break;
      }
    if (executable) {
      for (unsigned int j = 0; j < e.startEff.size(); j++) {
        if (e.startEff[j].var == v && e.startEff[j].value == value)
          return false;
      }
      for (unsigned int j = 0; j < e.endEff.size(); j++) {
        if (e.endEff[j].var == v && e.endEff[j].value == value)
          return false;
      }
    }
  }
...
bool LandmarkRPG::isExecutable(SASAction *a, std::vector<TVariable> *v,
  ...
  for (unsigned int j = 0; j < a->startEff.size(); j++) {
    for (unsigned int i = 0; i < n; i++)
      if (a->startEff[j].var == v->at(i) &&
          a->startEff[j].value == value->at(i))
        return false;
  }
  for (unsigned int j = 0; j < a->endEff.size(); j++) {
    for (unsigned int i = 0; i < n; i++)
  ...
  for (SASConditionalEffect &e : a->conditionalEff) {
    ...
    if (executable) {
      for (unsigned int j = 0; j < e.startEff.size(); j++) {
        for (unsigned int i = 0; i < n; i++)
          if (e.startEff[j].var == v->at(i) &&
              e.startEff[j].value == value->at(i))
            return false;
      }
      for (unsigned int j = 0; j < e.endEff.size(); j++) {
        for (unsigned int i = 0; i < n; i++)
          if (e.endEff[j].var == v->at(i) && e.endEff[j].value == value->at(i))
            return false;
      }
    }
  }
...
bool LandmarkRPG::allowedAction(SASAction *a,
...
void LandmarkRPG::addActionEffects(SASAction *a) {
  ...
  for (unsigned int j = 0; j < a->startEff.size(); j++) {
    code =
        SASTask::getVariableValueCode(a->startEff[j].var, a->startEff[j].value);
    if (!fluentAchieved(code)) {
  ...
  for (SASConditionalEffect &e : a->conditionalEff) {
    ...
    if (executable) {
      for (unsigned int j = 0; j < e.startEff.size(); j++) {
        code = SASTask::getVariableValueCode(e.startEff[j].var,
                                             e.startEff[j].value);
        if (!fluentAchieved(code)) {
          newLevel->push_back(code);
        }
      ...
      for (unsigned int j = 0; j < e.endEff.size(); j++) {
        code =
            SASTask::getVariableValueCode(e.endEff[j].var, e.endEff[j].value);
        if (!fluentAchieved(code)) {
          newLevel->push_back(code);
        }
      }
...
LandmarkTree::LandmarkTree(TState *state, SASTask *task,
...
void LandmarkTree::exploreRPG() {
  ...
  while (level > 0) {
    ...
    cout << "EXPLORING LEVEL " << level << ", whith " << objs[level].size()
         << " items" << endl;
#endif
    for (unsigned int i = 0; i < objs[level].size(); i++) {
      ...
      // actionProcessing is only launched if there are producers, that is, if A
      // is not an empty set
#ifdef DEBUG_LANDMARKS_ON
    ...
    for (unsigned int i = 0; i < disjObjs[level].size(); i++) {
      ...
      for (unsigned int j = 0; j < disjObj->fluentSet.size(); j++) {
        ...
        for (unsigned int k = 0; k < disjObj->fluentSet[j]->producers.size();
             k++) {
...
void LandmarkTree::actionProcessing(std::vector<SASAction *> *a, LTNode *g,
                                    int level) {
  if (a->size() == 0)
    return;
  ...
  for (unsigned int n = 0; n < i.size();
       n++) { // Exploring candidate landmarks in I
    LMFluent *p = i[n];
#ifdef DEBUG_LANDMARKS_ON
    ...
    if (verify(p)) {
      ...
      // The literal is stored only if it hasn't appeared before (it is ensured
      // by checking literalNode)
      LTNode *node;
...
#ifdef DEBUG_LANDMARKS_ON
        cout << "D2: " << d2->toString(task) << ", at level " << candidateLevel
             << endl;
#endif
      }
    } else {
...
void LandmarkTree::groupUSet(std::vector<USet *> *res,
                             std::vector<LMFluent *> *u,
                             std::vector<SASAction *> *a) {
  res->clear();
  ...
  // All the actions must have provided the uSet with at least a precondition of
  // each type
  for (unsigned int i = 0; i < aux.size(); i++) {
    USet *s = aux[i];
    int instances = 0;
...
void LandmarkTree::analyzeSet(USet *s, std::vector<SASAction *> *a,
                              std::vector<USet *> *u1) {
  std::vector<std::vector<LMFluent *>> fluentProducers(a->size());
  // Grouping the literals in the set according to the actions that generated
  // them
  for (unsigned int i = 0; i < a->size(); i++) {
    ...
    for (unsigned int j = 0; j < action->startCond.size(); j++) {
      if ((int)task->values[action->startCond[j].value].fncIndex == s->id) {
        int n = rpg.getFluentIndex(action->startCond[j].var,
                                   action->startCond[j].value);
    ...
    for (unsigned int j = 0; j < action->overCond.size(); j++) {
      if ((int)task->values[action->overCond[j].value].fncIndex == s->id) {
        int n = rpg.getFluentIndex(action->overCond[j].var,
                                   action->overCond[j].value);
        fluentProducers[i].push_back(rpg.getFluentByIndex(n));
      }
    }
    for (unsigned int j = 0; j < action->endCond.size(); j++) {
      if ((int)task->values[action->endCond[j].value].fncIndex == s->id) {
        int n = rpg.getFluentIndex(action->endCond[j].var,
                                   action->endCond[j].value);
        fluentProducers[i].push_back(rpg.getFluentByIndex(n));
      }
    }
    for (SASConditionalEffect &e : action->conditionalEff) {
...
void LandmarkTree::checkPreconditions(SASAction *a, int *common) {
  ...
  for (SASConditionalEffect &e : a->conditionalEff) {
    for (SASCondition &c : e.startCond) {
      int index = rpg.getFluentIndex(c.var, c.value);
      if (index >= 0 &&
          std::find(changed.begin(), changed.end(), index) == changed.end()) {
        common[index]++;
        changed.push_back(index);
      }
    }
...
int LandmarkTree::equalParameters(LMFluent *l,
                                  std::vector<LMFluent *> *actionFluents) {
  ...
  //  Check which candidate has most parameters of the same type than the target
  //  literal
  /*
...
void LandmarkTree::postProcessing() {
  // P: Candidate landmarks that precede necessarily a given candidate landmark
  // g
  ...
  for (unsigned int i = 0; i < nodes.size();
       i++) { // Only single literals are processed
    if (nodes[i]->single()) {
      for (unsigned int j = 0; j < nodes.size();
           j++) { // Check g column of the matrix to find literals l such that l
                  // <= n g
        if (matrix[j][i] && nodes[j]->single()) {
#ifdef DEBUG_LANDMARKS_ON
          cout << "PP: " << nodes[j]->toString(task) << " -> "
               << nodes[i]->toString(task) << endl;
#endif
          ...
          if (!verify(&a)) {
            ...
            while (ord < edges.size()) {
              ...
              if (e->node1->single() && e->node2->single() &&
                  e->node1->getIndex() == j && e->node2->getIndex() == i) {
...
void LandmarkTree::getActions(std::vector<SASAction *> *aList, LMFluent *l1,
                              LMFluent *l2) {
  aList->clear();
  if (rpg.getFluentByIndex(l1->index) == nullptr ||
      rpg.getFluentByIndex(l2->index) == nullptr)
    return;
  std::vector<SASAction *> &producers =
      task->producers[l2->variable][l2->value];
...
Landmarks::Landmarks(TState *state, SASTask *task,
                     std::vector<SASAction *> *tilActions) {
...
bool Landmarks::checkIndirectReachability(int orig, int current, int dst,

...
public:
  ...
  bool isEqual(USet *u) {
    if (id != u->id || fluentSet.size() != u->fluentSet.size())
      return false;
    for (unsigned int i = 0; i < fluentSet.size(); i++) {
...
public:
  ...
  std::string toString(SASTask *task) {
    if (singleLiteral)
      return "Node " + std::to_string(index) + ": " + fluent->toString(task);
    else
      return "Node " + std::to_string(index) + ": " +
             disjunction->toString(task);
  }
};
...
private:
  ...
  bool isExecutable(SASAction *a, std::vector<TVariable> *v,
...
public:
  ...
  bool verifyFluents(std::vector<TVariable> *v, std::vector<TValue> *value,
                     TState *s, SASTask *task);
  bool verifyActions(std::vector<SASAction *> *actions, TState *s,
                     SASTask *task);
};
...
private:
  ...
  void groupUSet(std::vector<USet *> *res, std::vector<LMFluent *> *u,
                 std::vector<SASAction *> *a);
  void analyzeSet(USet *s, std::vector<SASAction *> *a,
                  std::vector<USet *> *u1);
...
public:
  ...
  LandmarkTree(TState *state, SASTask *task,
...
public:
  ...
  void addAdjacent(LandmarkNode *nextNode) {
    std::vector<LandmarkNode *>::iterator it =
        find(nextNodes.begin(), nextNodes.end(), nextNode);
    if (it == nextNodes.end()) {
      nextNodes.push_back(nextNode);
    }
  }
  std::string toString(SASTask *task) {
    std::string res = "Node " + std::to_string(index) + ": (" +
                      task->variables[variables[0]].name + "," +
                      task->values[values[0]].name + ")";
    for (unsigned int i = 1; i < variables.size(); i++) {
      res += ",(" + task->variables[variables[i]].name + "," +
             task->values[values[i]].name + ")";
    }
    return res;
  }
  inline unsigned int numAdjacents() { return nextNodes.size(); }
  inline LandmarkNode *getAdjacent(unsigned int i) { return nextNodes[i]; }
  inline int getIndex() { return index; }
  void deleteAdjacent(unsigned int i) {
    nextNodes.erase(nextNodes.begin() + i);
  }
  unsigned int getNumFluents() { return variables.size(); }
  TVariable getVariable(unsigned int i) { return variables[i]; }
  TValue getValue(unsigned int i) { return values[i]; }
};
...
private:
  ...
  bool checkIndirectReachability(int orig, int current, int dst,

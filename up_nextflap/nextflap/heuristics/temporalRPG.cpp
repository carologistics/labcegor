...
void TemporalRPG::initialize(bool untilGoals, SASTask *task,
...
void TemporalRPG::addGoalToAchieve(SASCondition &c) {
  ...
  if (std::find(goalsToAchieve.begin(), goalsToAchieve.end(), v) ==
      goalsToAchieve.end())
...
void TemporalRPG::build(TState *state) {
  ...
  while (qPNormal.size() > 0) {
    ...
    cout << "EXTR.: " << fl->toString(task) << ", " << req.size()
         << " requirers" << endl;
#endif
      ...
      if (visitedAction[a->index] == 0) {
        if (verifyFluent && actionProducesFluent(a))
          ...
          for (unsigned int j = 0; j < a->startCond.size(); j++) {
            auxLevel = getFirstGenerationTime(a->startCond[j].var,
                                              a->startCond[j].value);
          ...
          if (applicable) {
            for (unsigned int j = 0; j < a->overCond.size(); j++) {
              auxLevel = getFirstGenerationTime(a->overCond[j].var,
                                                a->overCond[j].value);
              if (auxLevel < 0 || auxLevel > fl->level) {
                applicable = false;
                break; // Non applicable
            ...
            if (applicable) {
              ...
              for (unsigned j = 0; j < a->startEff.size(); j++) {
                ...
                if (auxLevel == -1 || auxLevel > effLevel) {
                  firstGenerationTime[SASTask::getVariableValueCode(v, value)] =
                      effLevel;
                  qPNormal.add(new FluentLevel(v, value, effLevel));
#ifdef DEBUG_TEMPORALRPG_ON
                  cout << "* PROG: (" << task->variables[v].name << ","
                       << task->values[value].name << ") -> " << effLevel
                       << endl;
#endif
                }
              }
        ...
        for (SASConditionalEffect &e : a->conditionalEff) {
          ...
          if (applicable) {
            ...
            for (SASCondition &c : e.startEff) {
              auxLevel = getFirstGenerationTime(c.var, c.value);
              if (auxLevel == -1 || auxLevel > effLevel) {
                firstGenerationTime[SASTask::getVariableValueCode(
                    c.var, c.value)] = effLevel;
...
void TemporalRPG::init(TState *state) {
  ...
  if (verifyFluent) {
    firstGenerationTime[SASTask::getVariableValueCode(
        fluentToVerify.variable, fluentToVerify.value)] = -1;
  }
  for (int i = 0; i < numActions; i++) {
...
#ifdef DEBUG_TEMPORALRPG_ON
        cout << "* PROG: (" << task->variables[v].name << ","
             << task->values[value].name << ") -> " << EPSILON << endl;
#endif
      }
    }
    ...
    for (SASConditionalEffect &e : a->conditionalEff) {
      ...
      if (hold) {
        for (SASCondition &c : e.startEff) {
          ...
          if (level == -1) {
            firstGenerationTime[SASTask::getVariableValueCode(c.var, c.value)] =
                EPSILON;
        ...
        for (SASCondition &c : e.endEff) {
          ...
          if (level == -1) {
            if (duration < 0)
              duration = EPSILON + task->getActionDuration(a, state->minState);
            firstGenerationTime[SASTask::getVariableValueCode(c.var, c.value)] =
                duration;
...
#ifdef DEBUG_TEMPORALRPG_ON
        cout << "* PROG: (" << task->variables[v].name << ","
             << task->values[value].name << ") -> " << duration << endl;
#endif
      }
    }
  }
...
bool TemporalRPG::checkAcheivedGoals() {
  while (goalsToAchieve.size() > 0 &&
         getFirstGenerationTime(goalsToAchieve[0]) >= 0) {
...
bool TemporalRPG::actionProducesFluent(SASAction *a) {
  for (unsigned int i = 0; i < a->startEff.size(); i++) {
    if (a->startEff[i].var == fluentToVerify.variable &&
        a->startEff[i].value == fluentToVerify.value)
      return true;
  }
  for (unsigned int i = 0; i < a->endEff.size(); i++) {
    if (a->endEff[i].var == fluentToVerify.variable &&
        a->endEff[i].value == fluentToVerify.value)
      return true;
  }
  for (SASConditionalEffect &e : a->conditionalEff) {
    for (SASCondition &c : e.startEff) {
...
void TemporalRPG::computeLiteralLevels() {
  ...
  int index = 0;
  for (auto it = firstGenerationTime.begin(); it != firstGenerationTime.end();
       ++it, ++index) {
    LMFluent f;
  ...
  while (qPNormal.size() > 0) {
    ...
    fluentLevels[i].push_back(
        SASTask::getVariableValueCode(fl->variable, fl->value));
    delete fl;
  }
}
...
#ifdef DEBUG_TEMPORALRPG_ON
    cout << "Action: " << task->actions[i].name << ", level " << actionLevels[i]
         << endl;
#endif
  }
  for (unsigned int i = 0; i < fluentList.size(); i++) {

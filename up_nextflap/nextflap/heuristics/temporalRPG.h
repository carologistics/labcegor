...
public:
  ...
  std::string toString(SASTask *task) {
    return "(" + task->variables[variable].name + "," +
...
public:
  ...
  std::string toString(SASTask *task) {
    return "[" + std::to_string(index) + "]: (" +
           task->variables[variable].name + "," + task->values[value].name +
           ") -> " + std::to_string(level);
  }
};
...
private:
  ...
  std::unordered_map<TVarValue, int>
      fluentIndex; // [variable,value] -> fluent index
  std::vector<std::vector<TVarValue>>
      fluentLevels; // List of fluents at each level
  ...
  inline float getFirstGenerationTime(TVarValue vv) {
    std::unordered_map<TVarValue, float>::const_iterator got =
        firstGenerationTime.find(vv);
...
public:
  void initialize(bool untilGoals, SASTask *task,
                  std::vector<SASAction *> *tilActions);
  ~TemporalRPG();
  ...
  inline unsigned int getFluentListSize() {
    return (unsigned int)fluentList.size();
  }
  inline unsigned int getNumFluentLevels() {
    return (unsigned int)fluentLevels.size();
  }
  inline int getFluentIndex(TVariable v, TValue value) {
    std::unordered_map<TVarValue, int>::const_iterator got =
        fluentIndex.find(SASTask::getVariableValueCode(v, value));
  ...
  inline int getLevelIndex(float level) {
    std::unordered_map<float, int>::const_iterator got =
        fluentLevelIndex.find(level);

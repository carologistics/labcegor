...
enum TermType { TERM_CONSTANT = 0, TERM_PARAMETER = 1, TERM_CONTROL_VAR = 2 };
...
public:
  ...
  inline bool equals(Term &t) { return type == t.type && index == t.index; }
};
...
public:
  ...
  NumericExpression(Symbol s, const std::vector<NumericExpression> &operands,
                    SyntaxAnalyzer *syn);
  std::string toString(const std::vector<Variable> &parameters,
                       const std::vector<Variable> &controlVars,
                       const std::vector<Function> &functions,
...
public:
  ...
  std::vector<GoalDescription> terms; // if type==GD_AND | GD_NOT | GD_OR |
                                      // GD_IMPLY | GD_EXISTS | GD_FORALL
...
public:
  ...
  std::vector<Precondition>
      terms; // if type==PT_AND | PT_NOT | PT_IMPLY | PT_EXISTS | PT_FORALL
  std::vector<Variable> parameters; // if type==PT_EXISTS | PT_FORALL
  GoalDescription
      goal; // if type==PT_F_CMP | PT_EQUALITY | PT_PREFERENCE | PT_GOAL
...
public:
  ...
  unsigned int addType(std::string name, std::vector<unsigned int> &parentTypes,
                       SyntaxAnalyzer *syn);
  unsigned int getObjectIndex(std::string const &name);
  unsigned int addConstant(std::string name, std::vector<unsigned int> &types,
                           SyntaxAnalyzer *syn);
  unsigned int addObject(std::string name, std::vector<unsigned int> &types,
                         SyntaxAnalyzer *syn);
  unsigned int getFunctionIndex(std::string const &name);
  unsigned int addPredicate(Function fnc, SyntaxAnalyzer *syn);
  unsigned int addFunction(Function fnc,
                           const std::vector<unsigned int> &valueTypes,
                           SyntaxAnalyzer *syn);
  unsigned int addFunction(Function fnc, SyntaxAnalyzer *syn);
  unsigned int addPreference(std::string name, const GoalDescription &goal,
                             SyntaxAnalyzer *syn);
  ...
  bool compatibleTypes(const std::vector<unsigned int> &types,
  ...
  unsigned int addAction(std::string name,
                         const std::vector<Variable> &parameters,
                         const Precondition &precondition, const Effect &effect,
                         SyntaxAnalyzer *syn);
  std::string toString();
  float ellapsedTime();

...
ParsedTask *Parser::parseDomain(char *domainFileName) {
  ...
  while (syn->isSym(token, Symbol::OPEN_PAR)) {
    syn->readColon();
    token = syn->readSymbol(
        9, Symbol::REQUIREMENTS, Symbol::TYPES, Symbol::CONSTANTS,
        Symbol::PREDICATES, Symbol::FUNCTIONS, Symbol::CONSTRAINTS,
        Symbol::ACTION, Symbol::DURATIVE_ACTION, Symbol::DERIVED);
    switch (token->symbol) {
...
ParsedTask *Parser::parseProblem(char *problemFileName) {
  ...
  while (syn->isSym(token, Symbol::OPEN_PAR)) {
    ...
    token = syn->readSymbol(7, Symbol::REQUIREMENTS, Symbol::OBJECTS,
                            Symbol::INIT, Symbol::GOAL, Symbol::CONSTRAINTS,
                            Symbol::METRIC, Symbol::LENGTH);
    switch (token->symbol) {
    case Symbol::REQUIREMENTS:
      parseRequirements();
      break;
...
void Parser::parseTypes() {
  ...
  while (!syn->isSym(token, Symbol::CLOSE_PAR)) {
    while (syn->isSym(token, Symbol::NAME)) {
      ...
      token =
          syn->readSymbol(3, Symbol::MINUS, Symbol::CLOSE_PAR, Symbol::NAME);
    }
...
// <type> ::= <primitive-type>
void Parser::parseParentTypes(std::vector<unsigned int> &types,
                              bool allowNumber) {
  Token *token = !allowNumber
                     ? syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::NAME)
                     : syn->readSymbol(3, Symbol::OPEN_PAR, Symbol::NUMBER_TYPE,
                                       Symbol::NAME);
  unsigned int index;
  ...
  do {
    ...
    if (!syn->isSym(token, Symbol::CLOSE_PAR)) {
      do {
        ...
        token = syn->readSymbol(3, Symbol::VARIABLE, Symbol::MINUS,
                                Symbol::CLOSE_PAR);
...
void Parser::parseControlVariableList(std::vector<Variable> &parameters) {
  ...
  do {
    ...
    if (!syn->isSym(token, Symbol::CLOSE_PAR)) {
      ...
      if (syn->isSym(token, Symbol::MINUS)) {
        Token *token =
            syn->readSymbol(2, Symbol::NUMBER_TYPE, Symbol::INTEGER_TYPE);
        if (syn->isSym(token, Symbol::NUMBER_TYPE))
          types.push_back(task->NUMBER_TYPE);
        else
          types.push_back(task->INTEGER_TYPE);
      } else
        types.push_back(task->getTypeIndex("#number"));
      for (unsigned int i = 0; i < varNames.size(); i++) {
        Variable v(varNames[i], types);
        parameters.push_back(v);
      }
...
// skeleton)>) <function typed list (x)> ::= x+ - <function type> <function
// typed list(x)> <function typed list (x)> ::= <function type>
// ::=:numeric-fluents number <function type> ::=:typing + :object-fluents
// <type>
  ...
  do {
    ...
    if (syn->isSym(token, Symbol::OPEN_PAR)) {
      do {
        ...
        aux = syn->readSymbol(3, Symbol::OPEN_PAR, Symbol::MINUS,
                              Symbol::CLOSE_PAR);
...
void Parser::parseDurativeAction() {
  ...
  task->addAction(name, parameters, controlVars, duration, condition, effect,
                  syn);
  syn->closePar();
}

// <duration-constraint> ::=:duration-inequalities (and
// <simple-duration-constraint>+) <duration-constraint> ::= ()
...
void Parser::parseDurationConstraint(vector<Duration> &duration,
                                     const vector<Variable> &parameters,
                                     const std::vector<Variable> &controlVars) {
  syn->openPar();
  Token *token =
      syn->readSymbol(6, Symbol::AND, Symbol::CLOSE_PAR, Symbol::LESS_EQ,
                      Symbol::GREATER_EQ, Symbol::EQUAL, Symbol::AT);
  if (syn->isSym(token, Symbol::CLOSE_PAR))
    return;
  if (syn->isSym(token, Symbol::AT)) {
    token = syn->readSymbol(2, Symbol::START, Symbol::END);
    TimeSpecifier time = syn->isSym(token, Symbol::START)
                             ? TimeSpecifier::AT_START
                             : TimeSpecifier::AT_END;
...
NumericExpression
Parser::parseNumericExpression(const vector<Variable> &parameters,
  ...
  if (syn->isSym(token, Symbol::AT) || syn->isSym(token, Symbol::NAME)) {
    ...
    syn->undoToken();
    unsigned int fncIndex = !syn->isSym(token, Symbol::AT)
                                ? task->getFunctionIndex(token->description)
  ...
  token = syn->readSymbol(6, Symbol::MINUS, Symbol::PLUS, Symbol::PROD,
                          Symbol::DIV, Symbol::AT, Symbol::NAME);
  if (syn->isSym(token, Symbol::AT) || syn->isSym(token, Symbol::NAME)) {
    vector<Term> fncParams;
    syn->undoToken();
    ...
    do {
      ...
      moreOperands = syn->isSym(aux, Symbol::NUMBER) ||
                     syn->isSym(aux, Symbol::OPEN_PAR) ||
                     syn->isSym(aux, Symbol::NAME) ||
                     syn->isSym(aux, Symbol::VARIABLE);
      if (moreOperands) {
        syn->undoToken();
...
unsigned int Parser::parseFunctionHead(vector<Term> &fncParams,
                                       const vector<Variable> &parameters) {
...
// <term> ::= <name>
// <term> ::= <variable>
Term Parser::parseTerm(const vector<unsigned int> &validTypes,
                       const vector<Variable> &parameters,
                       const std::vector<Variable> &controlVars) {
...
// <term> ::= <name>
// <term> ::= <variable>
Term Parser::parseTerm(const vector<Variable> &parameters,
                       const vector<Variable> &controlVars) {
...
// <da-GD> ::= <pref-timed-GD>
// <da-GD> ::= (and <da-GD>*)
// <da-GD> ::=:universal-preconditions (forall (<typed-list (variable)>)
// <da-GD>) <pref-timed-GD> ::= <timed-GD> <pref-timed-GD> ::=:preferences
// (preference [<pref-name>] <timed-GD>) <timed-GD> ::= (at <time-specifier>
// <GD>) <timed-GD> ::= (over <interval> <GD>)
DurativeCondition
Parser::parseDurativeCondition(const std::vector<Variable> &parameters,
                               const std::vector<Variable> &controlVars) {
  syn->openPar();
  Token *token =
      syn->readSymbol(6, Symbol::CLOSE_PAR, Symbol::AND, Symbol::FORALL,
                      Symbol::PREFERENCE, Symbol::AT, Symbol::OVER);
  DurativeCondition condition;
  if (syn->isSym(token, Symbol::CLOSE_PAR))
    ...
    condition.type = ConditionType::CT_AND;
    do {
      condition.conditions.push_back(
          parseDurativeCondition(parameters, controlVars));
    ...
    token = syn->readSymbol(2, Symbol::START, Symbol::END);
    condition.goal.time = syn->isSym(token, Symbol::START)
                              ? TimeSpecifier::AT_START
                              : TimeSpecifier::AT_END;
    ...
    condition.conditions.push_back(
        parseDurativeCondition(mergedParameters, controlVars));
    syn->closePar();
  } else { // preference
...
void Parser::parseGoalDescription(GoalDescription &goal,
                                  const std::vector<Variable> &parameters,
                                  const std::vector<Variable> &controlVars) {
  syn->openPar();
  Token *token = syn->readSymbol(
      12, Symbol::NOT, Symbol::AND, Symbol::OR, Symbol::IMPLY, Symbol::EXISTS,
      Symbol::FORALL, Symbol::EQUAL, Symbol::LESS, Symbol::GREATER,
      Symbol::GREATER_EQ, Symbol::LESS_EQ, Symbol::NAME);
...
// <GD> ::=:existential-preconditions exists (<typed list(variable)>) <GD>
// <GD> ::=:universal-preconditions forall (<typed list(variable)>) <GD>
void Parser::parseADLGoalDescription(GoalDescription &goal,
                                     const std::vector<Variable> &parameters,
                                     const std::vector<Variable> &controlVars) {
...
// <atomic formula(t)> ::=:equality = t t
// <f-comp> ::= <binary-comp> <f-exp> <f-exp>
void Parser::parseGoalDescriptionComparison(
    GoalDescription &goal, const std::vector<Variable> &parameters,
    const std::vector<Variable> &controlVars) {
  Token *token =
      syn->readSymbol(5, Symbol::EQUAL, Symbol::LESS, Symbol::GREATER,
  ...
  case Symbol::EQUAL:
    ...
    break;
  case Symbol::LESS:
    goal.comparator = CMP_LESS;
    break;
  case Symbol::GREATER:
    goal.comparator = CMP_GREATER;
    break;
  case Symbol::GREATER_EQ:
    goal.comparator = CMP_GREATER_EQ;
    break;
  case Symbol::LESS_EQ:
    goal.comparator = CMP_LESS_EQ;
    break;
  default:;
  }
  goal.type = GD_F_CMP;
...
// <da-effect> ::=:conditional-effects (forall (<typed list (variable)>)
// <da-effect>) <da-effect> ::=:conditional-effects (when <da-GD>
// <timed-effect>)
DurativeEffect
Parser::parseDurativeEffect(const std::vector<Variable> &parameters,
                            const std::vector<Variable> &controlVars) {
  syn->openPar();
  Token *token =
      syn->readSymbol(6, Symbol::AND, Symbol::FORALL, Symbol::WHEN, Symbol::AT,
                      Symbol::INCREASE, Symbol::DECREASE);
  ...
  case Symbol::AT:
    ...
    token = syn->readSymbol(2, Symbol::START, Symbol::END);
    effect.timedEffect.time = syn->isSym(token, Symbol::END)
                                  ? TimeSpecifier::AT_END
                                  : TimeSpecifier::AT_START;
  ...
  default:
    ...
    syn->undoToken();
    effect.assignment =
        parseAssignmentContinuousEffect(parameters, controlVars);
  }
  syn->closePar();
  return effect;
}

// <timed-effect> ::=:continuous-effects + :numeric-fluents (<assign-op-t>
// <f-head> <f-exp-t>) <assign-op-t> ::= increase <assign-op-t> ::= decrease
AssignmentContinuousEffect Parser::parseAssignmentContinuousEffect(
    const std::vector<Variable> &parameters,
    const std::vector<Variable> &controlVars) {
...
// <f-exp-t> ::= #t
ContinuousEffect
Parser::parseContinuousEffect(const std::vector<Variable> &parameters,
                              const std::vector<Variable> &controlVars) {
...
// <timed-effect> ::=:continuous-effects + :numeric-fluents (<assign-op-t>
// <f-head> <f-exp-t>) <f-assign-da> ::= (<assign-op> <f-head> <f-exp-da>)
...
void Parser::parseTimedEffect(TimedEffect &timedEffect,
                              const std::vector<Variable> &parameters,
                              const vector<Variable> &controlVars) {
  syn->openPar();
  Token *token =
      syn->readSymbol(9, Symbol::AND, Symbol::NOT, Symbol::INCREASE,
                      Symbol::DECREASE, Symbol::ASSIGN, Symbol::SCALE_UP,
  ...
  case Symbol::AT: {
    ...
    if (syn->isSym(token, Symbol::END) || syn->isSym(token, Symbol::START)) {
      timedEffect.time = syn->isSym(token, Symbol::END)
                             ? TimeSpecifier::AT_END
                             : TimeSpecifier::AT_START;
...
// <timed-effect> ::=:continuous-effects + :numeric-fluents <assign-op-t>
// <f-head> <f-exp-t> <p-effect> ::=:numeric-fluents <assign-op> <f-head>
// <f-exp> <p-effect> ::=:object-fluents assign <function-term> <term>
// <p-effect> ::=:object-fluents assign <function-term> undefined
FluentAssignment
Parser::parseFluentAssignment(const vector<Variable> &parameters,
                              const vector<Variable> &controlVars) {
  Token *token =
      syn->readSymbol(5, Symbol::INCREASE, Symbol::DECREASE, Symbol::ASSIGN,
                      Symbol::SCALE_UP, Symbol::SCALE_DOWN);
  FluentAssignment assignment;
  switch (token->symbol) {
  case Symbol::INCREASE:
    assignment.type = AS_INCREASE;
    break;
  case Symbol::DECREASE:
    assignment.type = AS_DECREASE;
    break;
  case Symbol::ASSIGN:
    assignment.type = AS_ASSIGN;
    break;
  case Symbol::SCALE_UP:
    assignment.type = AS_SCALE_UP;
    break;
  case Symbol::SCALE_DOWN:
    assignment.type = AS_SCALE_DOWN;
    break;
  default:;
  }
  vector<Term> fncParams;
  ...
  assignment.exp = parseEffectExpression(parameters, controlVars,
                                         syn->isSym(token, Symbol::ASSIGN));
  return assignment;
}

// <f-exp-da> | <f-exp-t> | <f-exp> | <term> (if isAssign) | undefined (if
// isAssign)
EffectExpression
Parser::parseEffectExpression(const vector<Variable> &parameters,
                              const vector<Variable> &controlVars,
                              bool isAssign) {
  Token *token =
      syn->readSymbol(5, Symbol::NUMBER, Symbol::OPEN_PAR, Symbol::VARIABLE,
                      Symbol::SHARP_T, Symbol::NAME);
  ...
  case Symbol::NAME:
    ...
    else if (isAssign && token->description.compare("undefined") == 0)
...
// <f-exp-t> ::= * <f-exp> #t
// <f-exp-t> ::= * #t <f-exp>
void Parser::parseEffectOperation(EffectExpression &exp,
                                  const vector<Variable> &parameters,
                                  const vector<Variable> &controlVars,
                                  bool isAssign) {
  ...
  if (syn->isSym(token, Symbol::NAME)) {
    ...
    do {
      Token *aux = syn->nextToken();
      moreOperands =
          syn->isSym(aux, Symbol::NUMBER) ||
          syn->isSym(aux, Symbol::OPEN_PAR) || syn->isSym(aux, Symbol::NAME) ||
          syn->isSym(aux, Symbol::VARIABLE) || syn->isSym(aux, Symbol::SHARP_T);
      syn->undoToken();
      if (moreOperands) {
        exp.operands.push_back(
            parseEffectExpression(parameters, controlVars, isAssign));
      }
    } while (moreOperands);
  }
}
...
// <pre-GD> ::= <pref-GD>
// <pre-GD> ::= (and <pre-GD>*)
// <pre-GD> ::=:universal-preconditions (forall (<typed list(variable)>)
// <pre-GD>) <pref-GD> ::=:preferences (preference [<pref-name>] <GD>) <pref-GD>
// ::= <GD>
Precondition
Parser::parsePrecondition(const vector<Variable> &parameters,
                          const std::vector<Variable> &controlVars) {
  Precondition prec;
  syn->openPar();
  Token *token = syn->readSymbol(
      13, Symbol::NOT, Symbol::AND, Symbol::OR, Symbol::IMPLY, Symbol::EXISTS,
      Symbol::FORALL, Symbol::EQUAL, Symbol::LESS, Symbol::GREATER,
      Symbol::GREATER_EQ, Symbol::LESS_EQ, Symbol::PREFERENCE, Symbol::NAME);
  bool readTerms = false;
  unsigned int numTerms = 0;
  switch (token->symbol) {
  case Symbol::NAME: // <atomic formula(term)>
...
// <effect> ::= (and <c-effect>*)
// <effect> ::= <c-effect>
// <c-effect> ::=:conditional-effects (forall (<typed list (variable)>)
// <effect>) <c-effect> ::=:conditional-effects (when <GD> <cond-effect>)
// <c-effect> ::= <p-effect>
// <p-effect> ::= (not <atomic formula(term)>)
// <p-effect> ::= <atomic formula(term)>
// <p-effect> ::=:numeric-fluents (<assign-op> <f-head> <f-exp>)
...
Effect Parser::parseEffect(const std::vector<Variable> &parameters,
  ...
  Effect eff;
  syn->openPar();
  Token *token = syn->readSymbol(10, Symbol::AND, Symbol::NOT, Symbol::INCREASE,
                                 Symbol::DECREASE, Symbol::ASSIGN,
                                 Symbol::SCALE_UP, Symbol::SCALE_DOWN,
                                 Symbol::FORALL, Symbol::WHEN, Symbol::NAME);
  switch (token->symbol) {
  case Symbol::AND:
    eff.type = ET_AND;
    token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
...
void Parser::parseObjects() {
  ...
  while (!syn->isSym(token, Symbol::CLOSE_PAR)) {
    while (syn->isSym(token, Symbol::NAME)) {
      ...
      token =
          syn->readSymbol(3, Symbol::NAME, Symbol::MINUS, Symbol::CLOSE_PAR);
    }
    if (syn->isSym(token, Symbol::MINUS)) {
      parseParentTypes(types, false);
      token = syn->readSymbol(2, Symbol::NAME, Symbol::CLOSE_PAR);
...
void Parser::parseInit() {
  ...
  float time;
  while (syn->isSym(token, Symbol::OPEN_PAR)) {
    token = syn->readSymbol(4, Symbol::EQUAL, Symbol::AT, Symbol::OVER,
                            Symbol::NAME);
    Fact f;
    if (syn->isSym(token, Symbol::EQUAL)) {
      ...
      if (!til) {
        ...
        if (syn->isSym(token, Symbol::NOT)) {
          ...
          f = parseFact();
          syn->closePar();
          f.value = f.value == task->CONSTANT_TRUE ? task->CONSTANT_FALSE
                                                   : task->CONSTANT_TRUE;
        } else {
          syn->undoToken();
          f = parseFact();
...
// <literal(name)> | <basic-function-term> <number> | <basic-function-term>
// <name>
Fact Parser::parseFact() {
  ...
  bool closePar = false;
  Token *token = syn->readSymbol(4, Symbol::OPEN_PAR, Symbol::AT, Symbol::OVER,
                                 Symbol::NAME);
  if (syn->isSym(token, Symbol::OPEN_PAR)) {
    closePar = true;
...
void Parser::parseMetric() {
  ...
  task->metricType =
      syn->isSym(token, Symbol::MINIMIZE) ? MT_MINIMIZE : MT_MAXIMIZE;
...
Metric Parser::parseMetricExpression() {
  Metric metric;
  Token *token =
      syn->readSymbol(9, Symbol::OPEN_PAR, Symbol::MINUS, Symbol::DIV,
                      Symbol::PROD, Symbol::PLUS, Symbol::NUMBER,
                      Symbol::TOTAL_TIME, Symbol::IS_VIOLATED, Symbol::NAME);
  switch (token->symbol) {
  case Symbol::OPEN_PAR:
...
unsigned int Parser::parseFluent(vector<unsigned int> &parameters) {
  ...
  if (functionIndex == MAX_UNSIGNED_INT ||
      !task->isNumericFunction(functionIndex))
  ...
  for (unsigned int i = 0; i < function.parameters.size(); i++) {
    ...
    if (!task->compatibleTypes(task->objects[objectIndex].types,
                               function.parameters[i].types))
...
// <pref-con-GD> ::=:universal-preconditions (forall (<typed list (variable)>)
// <pref-con-GD>) <pref-con-GD> ::=:preferences (preference [<pref-name>]
// <con-GD>) <pref-con-GD> ::= <con-GD> <con-GD> ::= (and <con-GD>*) <con-GD>
// ::= (forall (<typed list (variable)>) <con-GD>) <con-GD> ::= (at end <GD>)
// <con-GD> ::= (always <GD>)
// <con-GD> ::= (sometime <GD>)
...
Constraint Parser::parseConstraint(const vector<Variable> &parameters,
                                   const vector<Variable> &controlVars) {
  syn->openPar();
  Token *token = syn->readSymbol(
      13, Symbol::AND, Symbol::FORALL, Symbol::PREFERENCE, Symbol::AT,
      Symbol::ALWAYS, Symbol::SOMETIME, Symbol::WITHIN, Symbol::AT_MOST_ONCE,
      Symbol::SOMETIME_AFTER, Symbol::SOMETIME_BEFORE, Symbol::ALWAYS_WITHIN,
      Symbol::HOLD_DURING, Symbol::HOLD_AFTER);
  Constraint c;
  switch (token->symbol) {
  case Symbol::AND:
    c.type = RT_AND;
...
void Parser::parseConstraintGoal(Constraint &constraint,
                                 const vector<Variable> &parameters,
                                 const std::vector<Variable> &controlVars) {
  GoalDescription g;
  g.time = NONE;

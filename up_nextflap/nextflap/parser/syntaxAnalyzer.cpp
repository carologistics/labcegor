...
SyntaxAnalyzer::SyntaxAnalyzer() { buffer = NULL; }
  ...
  if (token != nullptr) {
    ...
    if (matchNumber(&value))
      ...
      if (description.at(0) == '?')
        ...
        unordered_map<string, Symbol>::const_iterator index =
            symbols.find(description);
...
bool SyntaxAnalyzer::matchNumber(float *value) {
  if (position >= bufferLength || buffer[position] < '0' ||
      buffer[position] > '9')
    return false;
  *value = 0;
  while (position < bufferLength && buffer[position] >= '0' &&
         buffer[position] <= '9') {
...
Token *SyntaxAnalyzer::readSymbol(Symbol s) { return readSymbol(1, s); }
...
string SyntaxAnalyzer::readName() {
  Token *res = nextToken();
  if (!isSym(res, Symbol::NAME) && !isSym(res, Symbol::AT) &&
      !isSym(res, Symbol::OVER) && !isSym(res, Symbol::OBJECTS) &&
      !isSym(res, Symbol::CONSTRAINTS)) {

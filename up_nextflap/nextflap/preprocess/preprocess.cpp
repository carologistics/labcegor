...
void Preprocess::preprocessOperators() {
  for (unsigned int i = 0; i < task->actions.size(); i++) {
    // cout << task->actions[i].toString(task->functions, task->objects,
    // task->types) << endl;
...
void Preprocess::checkPreconditionFeatures(Precondition &prec,
...
void Preprocess::checkPreconditionFeatures(DurativeCondition &prec,
                                           FeatureList *features) {
  switch (prec.type) {
  case CT_AND:
...
void Preprocess::checkEffectFeatures(DurativeEffect &eff,
                                     FeatureList *features) {
  switch (eff.type) {
  case DET_AND:
    for (unsigned int i = 0; i < eff.terms.size(); i++)
...
void Preprocess::checkGoalFeatures(DurativeCondition &goal,
                                   FeatureList *features, bool prec) {
  switch (goal.type) {
  case CT_AND:
...
void Preprocess::preprocessAction(Action a, FeatureList *features, bool isTIL,
                                  bool isGoal) {
  if (features->universalQuantifierPrec > 0 ||
      features->existentialQuantifierPrec > 0)
    removeQuantifiers(a.precondition, a.parameters.size());
  if (features->universalQuantifierEff > 0 ||
      features->existentialQuantifierEff > 0)
...
// Processes a durative action and stores the result in a vector of simplified
// operators
void Preprocess::preprocessAction(DurativeAction a, FeatureList *features,
...
void Preprocess::removeQuantifiers(Precondition &prec,
...
// Removes all quantifiers from a precondition
void Preprocess::removeQuantifiers(DurativeCondition &prec,
                                   unsigned int numParameters) {
  switch (prec.type) {
  case CT_AND:
    for (unsigned int i = 0; i < prec.conditions.size(); i++)
  ...
  case CT_FORALL:
    ...
    for (unsigned int i = 0; i < prec.conditions.size(); i++)
      removeQuantifiers(prec.conditions[i],
                        numParameters + prec.parameters.size());
    break;
  case CT_PREFERENCE:;
  }
}
...
void Preprocess::removeQuantifiers(Effect &eff, unsigned int numParameters) {
  // cout << eff.toString(borrar->parameters, borrar->parameters,
  // task->functions, task->objects, task->types) << endl;
  switch (eff.type) {
  case ET_AND:
    for (unsigned int i = 0; i < eff.terms.size(); i++)
...
// Removes all quantifiers from an effect
void Preprocess::removeQuantifiers(DurativeEffect &eff,
                                   unsigned int numParameters) {
  switch (eff.type) {
  case DET_AND:
    for (unsigned int i = 0; i < eff.terms.size(); i++)
...
void Preprocess::removeQuantifiers(GoalDescription &goal,
                                   unsigned int numParameters) {
  switch (goal.type) {
  case GD_AND:
  case GD_OR:
  ...
  for (unsigned int i = 0; i < task->objects.size(); i++) {
    if (task->compatibleTypes(task->objects[i].types, param.types)) {
      ...
      if (paramNumber + 1 < prec.parameters.size())
        replaceQuantifierParameter(prec, updatedTerm, paramNumber + 1,
                                   numParameters);
      else
  ...
  for (unsigned int i = 0; i < task->objects.size(); i++) {
    if (task->compatibleTypes(task->objects[i].types, param.types)) {
      ...
      if (paramNumber + 1 < eff.parameters.size())
        replaceQuantifierParameter(eff, updatedTerm, paramNumber + 1,
                                   numParameters);
      else
  ...
  for (unsigned int i = 0; i < task->objects.size(); i++) {
    if (task->compatibleTypes(task->objects[i].types, param.types)) {
      ...
      if (paramNumber + 1 < goal.parameters.size())
        replaceQuantifierParameter(goal, updatedTerm, paramNumber + 1,
                                   numParameters);
      else
...
void Preprocess::replaceParameter(Effect &eff, unsigned int paramToReplace,
...
// Replaces a parameter with a given object in an effect
void Preprocess::replaceParameter(DurativeEffect &eff,
...
// Replaces a parameter with a given object in an effect
void Preprocess::replaceParameter(TimedEffect &eff, unsigned int paramToReplace,
                                  unsigned int objectIndex) {
  switch (eff.type) {
  case TE_AND:
    for (unsigned int i = 0; i < eff.terms.size(); i++)
...
void Preprocess::replaceParameter(Precondition &term,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
  switch (term.type) {
  case PT_LITERAL:
...
// Replaces a parameter with a given object in a precondition
void Preprocess::replaceParameter(DurativeCondition &term,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
  switch (term.type) {
  case CT_AND:
...
void Preprocess::replaceParameter(Term &term, unsigned int paramToReplace,
                                  unsigned int objectIndex) {
...
void Preprocess::replaceParameter(Literal &literal, unsigned int paramToReplace,
                                  unsigned int objectIndex) {
...
void Preprocess::replaceParameter(GoalDescription &goal,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
  switch (goal.type) {
  case GD_LITERAL:
...
void Preprocess::replaceParameter(NumericExpression &exp,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
...
void Preprocess::replaceParameter(FluentAssignment &fa,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
...
void Preprocess::replaceParameter(EffectExpression &exp,
                                  unsigned int paramToReplace,
                                  unsigned int objectIndex) {
  switch (exp.type) {
  case EE_TERM:
...
// Removes negations and empty, unary ot nested conjunctions/disjunctions in a
// precondition
  ...
  case PT_OR:
    if (n == 0)
      ...
      if (parent == nullptr) {
        ...
        preconditionOptimization(&(parent->terms[termNumber]), parent,
                                 termNumber, a);
      }
  ...
  case CT_AND:
    if (n == 1) { // Unary conjuntion/disjunction
      ...
      if (parent == nullptr) {
        ...
        preconditionOptimization(&(parent->conditions[termNumber]), parent,
                                 termNumber, a);
      }
    } else { // Multiple conjuntion/disjunction
      for (unsigned int i = 0; i < n; i++)
...
void Preprocess::negationOptimization(Precondition *prec, Precondition *parent,
  ...
  case PT_NOT: // Double negation: ~~A = A
    if (parent == nullptr) {
      ...
      preconditionOptimization(&(parent->terms[termNumber]), parent, termNumber,
                               a);
    }
    break;
  case PT_F_CMP:
  ...
  case PT_EQUALITY:
    ...
    prec->goal = prec->terms[0].goal;
    prec->goal.type =
        prec->goal.type == GD_EQUALITY ? GD_INEQUALITY : GD_EQUALITY;
    prec->terms.clear();
    break;
  default:;
  }
...
// Removes negations and empty, unary or nested conjunctions/disjunctions in an
// effect
void Preprocess::effectOptimization(Effect *eff, Effect *parent,
                                    unsigned int termNumber, Action &a) {
...
// Removes negations and empty, unary or nested conjunctions/disjunctions in an
// effect
void Preprocess::effectOptimization(DurativeEffect *eff, DurativeEffect *parent,
                                    unsigned int termNumber,
                                    DurativeAction &a) {
  unsigned int n = eff->terms.size();
  switch (eff->type) {
...
// Removes negations and empty, unary or nested conjunctions/disjunctions in an
// effect
void Preprocess::effectOptimization(TimedEffect *eff, TimedEffect *parent,
                                    unsigned int termNumber,
  ...
  case TE_OR:
    if (n == 1) { // Unary conjuntion
      ...
      if (parent == nullptr) {
        ...
        parent->terms[termNumber] = child;
        effectOptimization(&(parent->terms[termNumber]), parent, termNumber,
                           nullptr);
      }
    } else { // Multiple conjuntion/disjunction
      for (unsigned int i = 0; i < n; i++)
  ...
  case CT_AND:
    if (n == 1) { // Unary conjuntion/disjunction
      ...
      if (parent == nullptr) {
        ...
        preconditionOptimization(&(parentEff->condition), nullptr, 0,
                                 parentEff);
      } else {
        parent->conditions[termNumber] = child;
        preconditionOptimization(&(parent->conditions[termNumber]), parent,
                                 termNumber, parentEff);
      }
    } else { // Multiple conjuntion/disjunction
      for (unsigned int i = 0; i < n; i++)
...
void Preprocess::negationOptimization(TimedEffect *eff, TimedEffect *parent,
                                      unsigned int termNumber,
                                      DurativeEffect *parentEff) {
  TimedEffect &child = eff->terms[0];
  switch (child.type) {
...
// Removes negations: eff is a negation in an effect
void Preprocess::negationOptimization(Effect *eff, Effect *parent,
                                      unsigned int termNumber, Action &a) {
  Effect &child = eff->terms[0];
  switch (child.type) {
  case ET_LITERAL:
...
// Removes negations and empty, unary or nested conjunctions/disjunctions in a
// goal description
void Preprocess::goalOptimization(GoalDescription *goal,
                                  Precondition *parentPrec, Effect *parentEff,
  ...
  case GD_OR:
    if (n == 0)
      ...
      if (parent != nullptr) {
        parent->terms[termNumber] = child;
        goalOptimization(&(parent->terms[termNumber]), nullptr, nullptr, parent,
                         termNumber);
...
void Preprocess::goalOptimization(GoalDescription *goal,
  ...
  case GD_OR:
    if (n == 0)
      ...
      if (parent != nullptr) {
        parent->terms[termNumber] = child;
        goalOptimization(&(parent->terms[termNumber]), nullptr, parent,
                         termNumber);
      } else {
        parentPrec->goal = child;
...
// Builds one operator (or more if there are disjunctions in the precondition)
// from the given action and stores them in prepTask->operators
  ...
  for (unsigned int i = 0; i < opList.size(); i++) {
    if (checkValidOperator(opList[i], a.parameters.size())) {
      terminateBuildingOperator(opList[i], a,
                                getOpName(a.name, i, opList.size()));
    }
  }
}
...
vector<Operator> Preprocess::buildOperatorPrecondition(Precondition &prec,
                                                       Action &a,
                                                       Operator *op) {
  ...
  case PT_NEG_LITERAL: {
    ...
    value.index =
        prec.type == PT_LITERAL ? task->CONSTANT_TRUE : task->CONSTANT_FALSE;
  ...
  case PT_OR:
    if (prec.terms.size() == 1) {
      ...
      for (unsigned int i = 0; i < prec.terms.size(); i++) {
        Operator newOp = *op;
        vector<Operator> partial =
            buildOperatorPrecondition(prec.terms[i], a, &newOp);
  ...
  case PT_F_CMP:
    ...
    break;
  case PT_EQUALITY:
    op->addEquality(prec.goal.eqTerms[0], prec.goal.eqTerms[1],
                    prec.goal.type == GD_EQUALITY);
    opList.push_back(*op);
    break;
  case PT_PREFERENCE: {
    OpPreference pref;
...
// Builds the operator precondition from the action precondition
vector<Operator> Preprocess::buildOperatorPrecondition(DurativeCondition &prec,
                                                       DurativeAction &a,
                                                       Operator *op) {
  vector<Operator> opList;
  // cout << "Aqui (" << prec.type << ") ";
...
// Builds the operator precondition from the action precondition
vector<Operator> Preprocess::buildOperatorPrecondition(GoalDescription &goal,
                                                       Action &a,
                                                       Operator *op) {
  vector<Operator> opList;
  switch (goal.type) {
  ...
  case GD_NEG_LITERAL: {
    ...
    value.type = TERM_CONSTANT;
    value.index =
        goal.type == GD_LITERAL ? task->CONSTANT_TRUE : task->CONSTANT_FALSE;
  ...
  case GD_OR:
    for (unsigned int i = 0; i < goal.terms.size(); i++) {
      Operator newOp = *op;
      vector<Operator> partial =
          buildOperatorPrecondition(goal.terms[i], a, &newOp);
      opList.insert(opList.begin(), partial.begin(), partial.end());
    }
    break;
  case GD_F_CMP:
...
// Builds the operator precondition from a conjunction in the action
// precondition
vector<Operator>
Preprocess::buildOperatorPreconditionAnd(Precondition &prec, Action &a,
                                         Operator *op, unsigned int numTerm) {
  vector<Operator> opList =
      buildOperatorPrecondition(prec.terms[numTerm], a, op);
  if (++numTerm >= prec.terms.size())
    return opList;
  vector<Operator> result;
  for (unsigned int i = 0; i < opList.size(); i++) {
    vector<Operator> partial =
        buildOperatorPreconditionAnd(prec, a, &opList[i], numTerm);
...
// precondition
vector<Operator>
Preprocess::buildOperatorPreconditionAnd(DurativeCondition &prec,
                                         DurativeAction &a, Operator *op,
                                         unsigned int numTerm) {
  vector<Operator> opList =
      buildOperatorPrecondition(prec.conditions[numTerm], a, op);
...
// precondition
vector<Operator>
Preprocess::buildOperatorPreconditionAnd(GoalDescription &goal, Action &a,
                                         Operator *op, unsigned int numTerm) {
  vector<Operator> opList =
      buildOperatorPrecondition(goal.terms[numTerm], a, op);
  if (++numTerm >= goal.terms.size())
    return opList;
  vector<Operator> result;
  for (unsigned int i = 0; i < opList.size(); i++) {
    vector<Operator> partial =
        buildOperatorPreconditionAnd(goal, a, &opList[i], numTerm);
    result.insert(result.end(), partial.begin(), partial.end());
  }
  return result;
}
...
// precondition
vector<Operator>
Preprocess::buildOperatorPreconditionAnd(GoalDescription &goal,
                                         DurativeAction &a, Operator *op,
                                         unsigned int numTerm) {
  vector<Operator> opList =
      buildOperatorPrecondition(goal.terms[numTerm], a, op);
...
bool Preprocess::checkValidOperator(Operator &op, unsigned int numParameters) {
  ...
  if (!setParameterValues(paramValues, equivalences, op.equality))
    ok = false;
  else if (!checkEqualities(paramValues, equivalences, op.equality,
                            numParameters))
    ok = false;
...
bool Preprocess::setParameterValues(unsigned int paramValues[],
  ...
  for (unsigned int i = 0; i < equality.size(); i++) {
    if (equality[i].equal) {
      ...
      if (equality[i].value1.type == TERM_PARAMETER &&
          equality[i].value2.type == TERM_CONSTANT) { // (= ?p X)
        if (paramValues[p1] != MAX_UNSIGNED_INT && paramValues[p1] != p2)
          return false;
        paramValues[p1] = p2;
      } else if (equality[i].value2.type == TERM_PARAMETER &&
                 equality[i].value1.type == TERM_CONSTANT) { // (= X ?p)
...
bool Preprocess::checkEqualities(unsigned int paramValues[],
  ...
  for (unsigned int i = 0; i < equality.size(); i++) {
    ...
    if (!equality[i].equal) {
      if (equality[i].value1.type == TERM_PARAMETER &&
          !equality[i].value2.type == TERM_PARAMETER) { // (!= ?p X)
        if (paramValues[p1] != MAX_UNSIGNED_INT && paramValues[p1] == p2)
          return false;
      } else if (equality[i].value2.type == TERM_PARAMETER &&
                 !equality[i].value1.type == TERM_PARAMETER) { // (!= X ?p)
        if (paramValues[p2] != MAX_UNSIGNED_INT && paramValues[p2] == p1)
          return false;
      } else if (equality[i].value1.type == TERM_PARAMETER) { // (!= ?p1 ?p2)
        if (paramValues[p1] != MAX_UNSIGNED_INT &&
            paramValues[p2] != MAX_UNSIGNED_INT &&
      ...
      if (equality[i].value1.type == TERM_PARAMETER &&
          equality[i].value2.type == TERM_PARAMETER) { // (= ?p1 ?p2)
        if (paramValues[p1] != MAX_UNSIGNED_INT &&
            paramValues[p2] != MAX_UNSIGNED_INT &&
            paramValues[p1] != paramValues[p2])
          return false;
...
bool Preprocess::checkPreconditions(unsigned int paramValues[],
                                    vector<OpFluent> &precs) {
  if (precs.size() != 0) {
...
void Preprocess::buildOperatorEffect(Operator &op, Effect &effect) {
  ...
  case ET_NEG_LITERAL: {
    ...
    value.type = TERM_CONSTANT;
    value.index =
        effect.type == ET_LITERAL ? task->CONSTANT_TRUE : task->CONSTANT_FALSE;
...
// Adds action effects to the operators
void Preprocess::buildOperatorEffect(Operator &op, TimedEffect &effect,
                                     TimeSpecifier time) {
  ...
  case TE_NEG_LITERAL: {
    ...
    value.type = TERM_CONSTANT;
    value.index =
        effect.type == TE_LITERAL ? task->CONSTANT_TRUE : task->CONSTANT_FALSE;
...
// Adds action effects to the operators
void Preprocess::buildOperatorEffect(Operator &op,
...
// Adds action effects to the operators
void Preprocess::buildOperatorEffect(Operator &op, FluentAssignment &effect,
                                     TimeSpecifier time) {
...
void Preprocess::buildOperatorEffect(Operator &op, GoalDescription &condition,
                                     Effect &effect) {
...
// Adds a conditional effect to the operator
void Preprocess::buildOperatorEffect(Operator &op, DurativeCondition &condition,
                                     TimedEffect &effect) {
  op.condEffects.emplace_back();
  OpConditionalEffect *cond = &op.condEffects.back();
  buildConditionalEffectCondition(&op, cond, &condition);
...
void Preprocess::buildConditionalEffectCondition(Operator *op,
                                                 OpConditionalEffect *cond,
  ...
  case GD_NEG_LITERAL: {
    Term value;
    value.type = TERM_CONSTANT;
    value.index = condition->type == GD_LITERAL ? task->CONSTANT_TRUE
                                                : task->CONSTANT_FALSE;
    OpFluent f;
    f.variable = condition->literal;
    f.value = value;
    if (condition->time == AT_END || condition->time == OVER_ALL)
      cond->atEnd.prec.push_back(f);
    else
  ...
  case GD_F_CMP: {
    if (condition->comparator == CMP_EQ &&
        ((condition->exp[0].type == NET_FUNCTION &&
          condition->exp[1].type == NET_TERM) ||
         (condition->exp[0].type == NET_TERM &&
          condition->exp[1].type == NET_FUNCTION))) { // Fluent
...
void Preprocess::buildConditionalEffectEffect(Operator *op,
                                              OpConditionalEffect *cond,
                                              Effect *effect) {
  switch (effect->type) {
  case ET_LITERAL:
...
void Preprocess::buildConditionalEffectEffect(Operator *op,
  ...
  case TE_NEG_LITERAL: {
    ...
    f.value = value;
    if (t == AT_END || t == NONE)
      cond->atEnd.eff.push_back(f);
    else
      cond->atStart.eff.push_back(f);
  } break;
  case TE_ASSIGNMENT:
    if (effect->assignment.type == AS_ASSIGN &&
        effect->assignment.exp.type == EE_TERM) {
      OpFluent f;
      ...
      OpEffect e(effect->assignment);
      if (t == AT_END || t == NONE)
        cond->atEnd.numericEff.push_back(e);
      else
        cond->atStart.numericEff.push_back(e);
    }
    break;

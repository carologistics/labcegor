...
MutexGraph::MutexGraph() { numVertex = 0; }
...
void MutexGraph::computeMutexComponent(unsigned int origin,
...
void MutexGraph::depthFirstSearch(unsigned int origin,
...
void MutexGraph::processNonMutexComponent(
    const std::vector<unsigned int> &component) {
...
unsigned int
MutexGraph::highestDegreeVertex(const std::vector<unsigned int> &component) {
  unsigned int v = 0;
  ...
  for (unsigned int i = 1; i < component.size(); i++)
    if (component[v] < numVertex &&
        adjacent[component[i]].size() > adjacent[component[v]].size())
      v = i;
  return component[v];
}

// Computes a mutually exclusive subcomponent which contains vertex v1 and v2
void MutexGraph::computeMutexSubcomponent(
    unsigned int v1, unsigned int v2, std::vector<unsigned int> &subcomponent) {
  bool *visited = new bool[adjacent.size()]{false};
...
// Removes the links of the first element in the subcomponent with the other
// vertex
...
unsigned int MutexGraph::numVariables() { return mutexComponents.size(); }

void MutexGraph::getVariable(unsigned int index,
                             std::vector<unsigned int> &values,
                             unsigned int noneOfThose) {
  values.clear();

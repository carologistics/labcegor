...
private:
  ...
  std::vector<std::vector<unsigned int>>
      mutexComponents; // Set of mutually exclusive connected components
  std::vector<unsigned int>
      variableIndex; // Graph vertex -> real variable index
  std::unordered_map<unsigned int, unsigned int>
      vertexIndex; // Real variable index -> graph vertex
  std::vector<std::vector<unsigned int>>
      adjacent; // Adjacent vertex for each vertex
  void depthFirstSearch(unsigned int origin,
                        std::vector<unsigned int> &component, bool *visited);
  ...
  void computeMutexSubcomponent(unsigned int v1, unsigned int v2,
  ...
  void computeMutexComponent(unsigned int origin,
...
public:
  ...
  void getVariable(unsigned int index, std::vector<unsigned int> &values,

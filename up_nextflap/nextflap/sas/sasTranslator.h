...
public:
  ...
  std::vector<std::vector<unsigned int>>
      literals; // literal -> <sasVariable, sasValue>
...
private:
  ...
  bool holdsCondition(const GroundedCondition *c,
  ...
  inline static int findInVector(unsigned int value,
  ...
  inline void addMutex(unsigned int v1, unsigned int v2) {
    if (!mutex[v1][v2]) {
      ...
      std::unordered_map<unsigned long long, bool>::const_iterator got =
          mutexChanges.find(code);
  ...
  void updateDomain(SASTask *sTask, MutexGraph *graph,
  ...
  void createNumericAndFiniteDomainVariables(SASTask *sTask,
                                             LiteralTranslation *trans);
  void setInitialValuesForVariables(SASTask *sTask, LiteralTranslation *trans);
  void createAction(GroundedAction *ga, SASTask *sTask,
                    LiteralTranslation *trans, bool isGoal);
  void generateControlVar(SASAction *a, GroundedControlVar *cv);
  void generateDuration(SASAction *a, GroundedDuration *gd,
                        LiteralTranslation *trans);
  char generateComparator(int comp);
  char generateTime(int time);
  SASNumericExpression generateNumericExpression(GroundedNumericExpression *gn,
                                                 LiteralTranslation *trans);
  SASNumericExpression
  generateNumericExpression(PartiallyGroundedNumericExpression *gn,
                            LiteralTranslation *trans);
  char generateNumericExpressionType(int type);
  char generatePartiallyNumericExpressionType(int type);
  void generateCondition(GroundedCondition *cond, SASTask *sTask,
                         LiteralTranslation *trans,
  ...
  bool modifiedVariable(unsigned int sasVar,
                        ...
                        unsigned int effIndex, LiteralTranslation *trans);
  void checkModifiedVariable(SASCondition *c, SASAction *a);
  void checkNegatedPreconditionLiterals(GroundedAction *a);
  SASNumericCondition generateNumericCondition(GroundedNumericCondition *cond,
                                               LiteralTranslation *trans,
                                               SASAction *a);
  SASNumericEffect generateNumericEffect(GroundedNumericEffect *cond,
                                         LiteralTranslation *trans);
  char generateAssignment(int assignment);
  SASPreference generatePreference(GroundedPreference *pref, SASTask *sTask,
                                   LiteralTranslation *trans);
  SASGoalDescription generateGoalDescription(GroundedGoalDescription *gd,
                                             SASTask *sTask,
                                             LiteralTranslation *trans);
  SASConstraint createConstraint(GroundedConstraint *gc, SASTask *sTask,
                                 LiteralTranslation *trans);
  ...
  void classifyEffect(GroundedCondition &e, std::vector<unsigned int> &newA,
                      std::vector<unsigned int> &add,
                      std::vector<unsigned int> &del);

public:
  SASTask *translate(GroundedTask *gTask, bool onlyGenerateMutex,
